...
lept_value* lept_insert_array_element(lept_value* v, size_t index) {
	...
	if (cap == size)
		lept_reserve_array(v, cap == 0? 1 : cap * 2);
	// swap
	if (size > 0)
	{
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    /* \done */
	const size_t index = lept_find_object_index(v, key, klen);
	if (index == LEPT_KEY_NOT_EXIST)
	{
		// capacity
		const size_t size = v->u.o.size;
		const size_t cap = v->u.o.capacity;
		if (size == cap)
		{
			lept_reserve_object(v, cap == 0? 1 : 2 * cap);
		}
        memcpy(v->u.o.m[size].k = (char*)malloc(klen + 1), key, klen);
